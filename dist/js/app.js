(() => {
        ...
        if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
                ...
                if (el.closest("summary") && el.closest("[data-spollers]").classList.contains("_spoller-init")) {
                    ...
                    e.preventDefault();
                    let items = document.querySelectorAll(".spollers__item");
                    items.forEach((item => {
                        if (item.hasAttribute("open")) item.classList.toggle("active-1");
                    }));
                }
    ...
    function initSliders() {
        if (document.querySelector(".our-team__slider.swiper")) new swiper_core_Swiper(".our-team__slider.swiper", {
        ...
        if (document.querySelector(".testing__slider.swiper")) new swiper_core_Swiper(".testing__slider.swiper", {
    ...
    let service = document.querySelector("#type_icon");
    let service2 = document.querySelector("#type_icon2");
    let prof = document.querySelector("#proffesional_type");
    let accessib = document.querySelector("#accessibility_type");
    document.querySelector(".accessibility_type");
    if (prof || accessib) {
        prof.addEventListener("click", (function addClass() {
            service.classList.toggle("visible");
            prof.classList.toggle("clickable");
        }));
        accessib.addEventListener("click", (function() {
            service2.classList.toggle("visible");
            accessib.classList.toggle("clickable");
        }));
        document.addEventListener("click", (function(event) {
            if (!event.target.closest(".clickable")) {
                service.classList.remove("visible");
                prof.classList.remove("clickable");
                service2.classList.remove("visible");
                accessib.classList.remove("clickable");
            }
        }));
    }
    ...
    console.log(p);
    let start = 0;
    let script_button = document.querySelector("#btn");
    let show = document.querySelector(".show");
    let design = document.querySelector(".desig");
    let brand = document.querySelector(".branding");
    let illust = document.querySelector(".illustr");
    let motion = document.querySelector(".motion");
    let seeAll = document.querySelector("#see-all");
    let seeAllPost = document.querySelector("#see-all-post");
    let buttons = [ script_button, show ];
    let script_select = document.querySelector(".select");
    buttons.forEach((el => {
        if (el) el.addEventListener("click", callProducts);
    }));
    if (design) design.addEventListener("click", callDesign);
    if (brand) brand.addEventListener("click", callBrand);
    if (illust) illust.addEventListener("click", callIllust);
    if (motion) motion.addEventListener("click", callMotion);
    if (script_select) script_select.addEventListener("change", changeSelect);
    if (seeAllPost) seeAllPost.addEventListener("click", getTest);
    let ev = document.querySelector(".event-button");
    let arrey = [ "_one ", "_two ", "_tree ", "_four ", "_one ", "_two ", "_tree ", "_four ", "_one ", "_two ", "_tree ", "_four ", "_one ", "_two " ];
    const portfolioImages = document.querySelector(".portfolio_images");
    if (ev) ev.addEventListener("click", getEvents);
    if (seeAll) seeAll.addEventListener("click", getTest);
    function callProducts(event) {
        let items = document.querySelectorAll("#portfolio");
        let file = "";
        start = 0;
        if (items.length == 0) {
            ...
            file = "http://192.168.1.13:8080/json/portfolioBasic.json";
        } else file = "http://192.168.1.13:8080/json/portfolio.json";
        if (event) {
            event.preventDefault();
            getProduct(file, event);
        } else getProduct(file, script_select[script_select.selectedIndex]);
    }
    async function getProduct(file, event) {
        let arrey = [ "_one ", "_two ", "_tree ", "_four ", "_one ", "_two ", "_tree ", "_four ", "_one ", "_two ", "_tree ", "_four ", "_one ", "_two " ];
        document.querySelector(".portfolio_images");
        for (let j = 0; j < arrey.length; j++) {
            ...
            if (response.ok) {
                ...
                if (event.target == script_button || event.target == show) loadProducts(result.portfolio, arrey[j], event); else {
                    console.log("Hello");
                    loadProductsLinks(result.portfolio);
                }
            } else alert("ERROR");
        }
    }
    function loadProductsLinks(data) {
        data.forEach((item => {
            for (let j = start; j < arrey.length; j++) {
                if (start == data.length) break;
                const productId = item.id;
                const chapter = item.chapter;
                const text = item.text;
                const img = item.image;
                let portStart = `<div id ="${productId}"class="image${arrey[j]}_ibg">`;
                let portEnd = "</div>";
                let chapterText = `<div class="chapter_one">${chapter}</div>`;
                let portName = `<h3 class="chapter_name">${text}</h3>`;
                let imgPath = `<img src=${img} alt="">`;
                portStart += chapterText;
                portStart += portName;
                portStart += imgPath;
                portStart += portEnd;
                portfolioImages.insertAdjacentHTML("beforeend", portStart);
                start++;
                break;
            }
        }));
    }
    function loadProducts(data, index, e) {
        for (let k = start; k < data.length; k++) {
            const productId = data[k].id;
            const chapter = data[k].chapter;
            const text = data[k].text;
            const img = data[k].image;
            let portStart = `<div id ="${productId}"class="image${index}_ibg">`;
            let portEnd = "</div>";
            let chapterText = `<div class="chapter_one">${chapter}</div>`;
            let portName = `<h3 class="chapter_name">${text}</h3>`;
            let imgPath = `<img src=${img} alt="">`;
        ...
        if (start === data.length) script_button.remove();
        let items = document.querySelectorAll("#portfolio");
        console.log(items.length);
    }
    function callDesign(event) {
        let file = "http://192.168.1.13:8080/json/design.json";
        let arr = document.querySelectorAll("._ibg");
        arr.forEach((el => {
            el.remove();
        }));
        ...
        if (event) {
            ...
            console.log("444");
            getProduct(file, script_select[script_select.selectedIndex]);
        }
    }
    function callBrand(event) {
        let file = "http://192.168.1.13:8080/json/branding.json";
        let arr = document.querySelectorAll("._ibg");
        arr.forEach((el => {
            el.remove();
            script_button.remove();
    ...
    function callIllust(event) {
        let file = "http://192.168.1.13:8080/json/illustration.json";
        let arr = document.querySelectorAll("._ibg");
        arr.forEach((el => {
            el.remove();
            script_button.remove();
    ...
    function callMotion(event) {
        let file = "http://192.168.1.13:8080/json/motion.json";
        let arr = document.querySelectorAll("._ibg");
        arr.forEach((el => {
            el.remove();
        }));
    ...
    function changeSelect() {
        ...
        if (script_select[script_select.selectedIndex].className === "design") callDesign(); else if (script_select[script_select.selectedIndex].className === "branding") callBrand(); else if (script_select[script_select.selectedIndex].className === "illustr") callIllust(); else if (script_select[script_select.selectedIndex].className === "motion") callMotion(); else callProducts();
    }
    async function getEvents() {
        let file = "http://192.168.1.13:8080/json/events.json";
        let ev = document.querySelector(".event-button");
        let response = await fetch(file, {
            method: "GET"
        });
    ...
    function showEvents(data) {
        data.events.forEach((element => {
            let id = element.id;
            let months = element.months;
            let hours = element.hours;
            let title = element.title;
            let text = element.text;
            let eventProduct = `<div class="events_item">\n   <div class="data">\n   <div class="number_data">${id}</div>\n   <div class="month_data">${months}</div></div>\n   <div class="data-hours">\n      \n   <div class="hours">${hours}</div>\n</div>\n<span></span>\n<h4 class="about_topic"${title}</h4>\n<div class="text_p">${text}</div>\n<div class="event-btn">\n<a href ="" >Explore Now</a></div>\n\n</div>`;
            let exp = document.querySelector(".explore_items");
            exp.insertAdjacentHTML("beforeend", eventProduct);
        }));
    }
    async function getTest() {
        let file = "http://192.168.1.13:8080/json/test.json";
        let response = await fetch(file, {
            method: "GET"
        });
        let seeAll = document.querySelector("#see-all");
        if (response.ok) {
            ...
            if (seeAllPost) seeAllPost.remove();
        } else console.log("ERROR");
    }
    function showTest(data) {
        data.test.forEach((item => {
            ...
            let testProduct = `<div class=" client-post">\n      <div class="stars">\n         <img src="img/icons/test/Star.png" alt="">\n         <img src="img/icons/test/Star.png" alt="">\n         <img src="img/icons/test/Star.png" alt="">\n         <img src="img/icons/test/Star.png" alt="">\n         <img src="img/icons/test/Star.png" alt="">\n      </div>\n      <div class="text_p">${text}</div>\n      <div class="twit">\n                  <div class="twit_image">\n            <img src=${photo} alt="">\n         </div>\n         \n         <div class="twit_name">${name}</div>\n         <div class="twit_text">${position}</div>\n      \n      </div>\n\n   </div>`;
            let columnTwo = document.querySelector(".column_two");
            let columnOne = document.querySelector(".column_one");
            if (!columnOne && !columnTwo) {
                let place = document.querySelector(".test_clients");
                place.insertAdjacentHTML("beforeend", testProduct);
            } else if (item.id == "column_one") columnOne.insertAdjacentHTML("beforeend", testProduct); else columnTwo.insertAdjacentHTML("beforeend", testProduct);
        }));
    }
    let personButton = document.querySelector("#person_button");
    if (personButton) {
        personButton.addEventListener("click", (async function(event) {
            event.preventDefault();
            let file = "http://192.168.1.13:8080/json/portfolio.json";
            let response = await fetch(file, {
                method: "GET"
            });
            if (response.ok) {
        ...
        function showWorks(data) {
            let place = document.querySelector(".portfolio_image.image");
            data.portfolio.forEach((item => {
                item.id;
                let chapter = item.chapter;
                let text = item.text;
                let image = item.image;
                let personProduct = `\n         <div class="image_one _ibg">\n\t\t\t\t\t\t\t<div class="image image_content">\n\t\t\t\t\t\t\t<div id = "portfolio" class="chapter_one">${chapter}</div>\n\t\t\t\t\t\t\t<h3 class="chapter_name">${text}</h3></div>\n\t\t\t\t\t\t\t<div class="image image_image">\n\t\t\t\t\t\t\t<img src="${image}" alt=""></div>\n\t\t\t\t\t\t</div>\n         `;
                place.insertAdjacentHTML("beforeend", personProduct);
            }));
        }
    }
    window["FLS"] = true;
